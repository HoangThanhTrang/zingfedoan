<template>
  <div class="container">
    <div class="banner-top100" style="display: flex;justify-content: center;align-items: center;">
    ...
    <div id="zmp3_Top">
      <div
        class="z2-zads-zone"
        id="adtima-zone-2281335701556915071"
        z2-zoneid="2281335701556915071"
        z2-width="970"
        z2-height="1"
        z2-close="true"
        z2-close-freq="-1"
        z2-close-ignore="remove"
        z2-logo="no_fb"
        z2-reqid="850449"
      >
        <iframe
          id="adtimaIFrameWrapper_2281335701556915071"
          scrolling="no"
          marginwidth="0"
          marginheight="0"
          frameborder="0"
        ...
        ></iframe>
        <div id="adtimaDivWrapper_2281335701556915071" style="display: none"></div>
      </div>
    </div>
    <div class="zm-section playlist-section mar-b-30" v-for="(item, index) in data" :key="index">
      <div class="container">
        <h3 class="zm-section-title title is-2">{{item.title}}</h3>
        <div class="zm-carousel-wrapper" v-if="index === 0">
          <div class="zm-carousel">
            <div class="zm-carousel__container" style="transform: translate3d(0px, 0px, 0px)">
              <div class="zm-carousel-item is-fullhd-20 is-widescreen-20 is-desktop-3 is-touch-3 is-tablet-3" v-for="(record, keyRecord) in item.items" :key="keyRecord">
                  ...
                  <div class="zm-card">
                    <div>
                      <a
                        ...
                        href="/album/Top-100-Bai-Hat-Nhac-Tre-Hay-Nhat-Hong-Thanh-DJ-Mie-Dinh-Tung-Huy-Thieu-Bao-Tram/ZWZB969E.html"
                      >
                    ...
                    <div class="zm-card-content">
                      <h4 class="title is-6">
                        <a
                          ...
                          ><span
                            ><span
                              ><span><span>{{record.title}}</span></span></span
                            ><span style="position: fixed; visibility: hidden; top: 0px; left: 0px">…</span></span
                          ></a
                        >
                      </h4>
                      <h3 class="mt-10 subtitle">
                        <a class="is-ghost" href="/Hong-Thanh" v-for="(artist, keyArtist) in record.artists" :key="keyArtist">
                          <template v-if="keyArtist <= 2">
                            <template v-if="keyArtist <= 1">
                              {{artist.name}},
                            </template>
                            <template v-else>
                              {{artist.name}}...
          ...
          <button class="zm-btn zm-carousel-control-prev zm-disabled is-hide button" tabindex="0"><i class="icon ic-go-left"></i></button>
          <button class="zm-btn zm-carousel-control-next is-hide button" tabindex="0"><i class="icon ic-go-right"></i></button>
        </div>
        <div class="columns is-multiline" v-else>
          <div class="column mar-b-30 is-fullhd-20 is-widescreen-20 is-desktop-3 is-touch-3 is-tablet-3" v-for="(record, keyRecord) in item.items" :key="keyRecord">
            <div class="playlist-wrapper is-normal">
              <div class="zm-card">
                <div>
                  <a class="" title="Top 100 Nhạc Audiophile Âu Mỹ Hay Nhất" href="/album/Top-100-Nhac-Audiophile-Au-My-Hay-Nhat-Bobby-Vinton-Herman-s-Hermits-Matt-Monro-Paul-Anka/ZWZB96D8.html">
                    <div class="zm-card-image">
                      <figure class="image is-48x48">
                        <img :src="record.thumbnail" alt="" >
                      </figure>
                ...
                <div class="zm-card-content">
                  <h4 class="title is-6"><a class="" title="Top 100 Nhạc Audiophile Âu Mỹ Hay Nhất" href="/album/Top-100-Nhac-Audiophile-Au-My-Hay-Nhat-Bobby-Vinton-Herman-s-Hermits-Matt-Monro-Paul-Anka/ZWZB96D8.html">
                    <span><span><span><span>{{record.title}}</span></span></span><span style="position: fixed; visibility: hidden; top: 0px; left: 0px;">…</span></span></a></h4>
                  <h3 class="mt-10 subtitle">
                    <a class="is-ghost" href="/Hong-Thanh" v-for="(artist, keyArtist) in record.artists" :key="keyArtist">
                      <template v-if="keyArtist <= 2">
                        <template v-if="keyArtist <= 1">
...
<script lang="ts">
  ...
  const top100Repository: Top100Repository = getRepository('top-100')

  @Component({
    components: {}
  })
  export default class Top100 extends Vue {
    ...
    created(): void {
      top100Repository.getTop100().then((res: any) => {
        this.data = [...res]
...
<style scoped>
  ...
  .playlist-wrapper .zm-card-content .title a {
    color: #fff;
  }
